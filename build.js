...
:root.theme-dark .btn-primary, :root.theme-auto .btn-primary { color: #fff; }
:root.theme-dark .btn-primary:hover, :root.theme-auto .btn-primary:hover { color: #fff; }
.btn-secondary {
...
.sr-preferred-btn {
  ...
  transition: background 200ms cubic-bezier(.65,0,.35,1),
              transform 200ms cubic-bezier(.65,0,.35,1),
              box-shadow 200ms cubic-bezier(.65,0,.35,1);
  margin-top: .75rem;
}
.sr-preferred-btn:hover, .sr-preferred-btn:focus-visible {
  background: #34A853; transform: translateY(-1px);
  box-shadow: 0 6px 18px rgba(52,168,83,.35);
}
.sr-preferred-btn:active { transform: translateY(0); }
.sr-preferred-btn:focus-visible { outline: 2px solid #4285F4; outline-offset: 2px; }
.sr-preferred-btn svg { flex: none; display: block; }
.sr-preferred-btn--added, .sr-preferred-btn--added:hover { background: #34A853; }
`;
...
const SHARED_HEAD = ({ title, description, canonical, slug, pageType = 'website' }) => `
  ...
  <script defer src="${B}js/preferred-source.js"></script>
`;
...
<footer class="site-footer" role="contentinfo">
  <div class="footer-grid">
    <div class="footer-brand">
      ...
      <button type="button" data-sr-preferred-source class="sr-preferred-btn"
              aria-label="Add Signed Reviews as a preferred source on Google">
        ...
        <span class="sr-preferred-label">Add us as a preferred source on Google</span>
      </button>
    </div>
    <div class="footer-col">
      <strong class="footer-heading">Product</strong>
      <ul>
...
function buildComparison() {
  const body = `<article class="prose">
    ...
    <p>In this detailed signed reviews vs trustpilot comparison, we examine two very different approaches to online reviews. Trustpilot, the most recognised public review platform, and Signed Reviews, the first platform to require every review be cryptographically linked to a completed Stripe transaction. Understanding the trade-offs helps you decide which aligns with your trust and authenticity goals.</p>
...
function buildLearnFakeReviewsWork() {
  ...
  const body = `<article class="prose">
    ...
    <ul>
      <li><strong>Trustpilot removed 4.5 million fake reviews in 2024</strong>, which is 7.4% of all submissions that year.</li>
...
const TOOL_STYLES = `
  ...
  .frc-run:hover { transform: translateY(-1px); box-shadow: 0 6px 18px rgba(179,157,69,.44); }
  .frc-sample {
    display: inline-flex; align-items: center; justify-content: center;
  ...
  .frc-sample:hover { background: var(--navy-50); color: var(--navy-900); }
  :root.theme-dark .frc-sample, :root.theme-auto .frc-sample {
    color: var(--navy-100); border-color: rgba(255,255,255,.18); background: rgba(255,255,255,.02);
  }
  ...
  .frc-summary { display: flex; align-items: baseline; gap: 1rem; flex-wrap: wrap; }
  .frc-score {
    font-family: 'Instrument Serif', Georgia, serif;
    font-size: 2.75rem; line-height: 1; letter-spacing: -0.01em;
    color: var(--navy-900);
  }
  :root.theme-dark .frc-score, :root.theme-auto .frc-score { color: #fff; }
  .frc-meta { flex: 1; min-width: 12rem; }
  .frc-meta strong { font-size: .95rem; }
  .frc-meta div { font-size: .85rem; color: var(--muted); }
  .frc-meter { height: 8px; background: var(--border); border-radius: 4px; overflow: hidden; margin-top: .9rem; }
  .frc-meter-fill {
  ...
  .frc-signal.bad { background: rgba(214,69,69,.10); border-left-color: #d64545; }
  .frc-signal.good { background: rgba(63,148,95,.10); border-left-color: #3f945f; }
  .frc-signal.note { background: rgba(212,196,102,.12); border-left-color: var(--gold-500); }
...
function buildFakeReviewChecker() {
  ...
  (function () {
    ...
    function analyze() {
      ...
      if (!t) {
        output.innerHTML = '<p class="frc-empty">Paste review text first, then press Check.</p>';
        return;
      }
      ...
      if (wordCount < 25) {
        output.innerHTML = '<div class="frc-result"><div class="frc-summary"><span class="frc-score">—</span>' +
          '<div class="frc-meta"><strong>Too short to assess</strong><div>Fewer than 25 words.</div></div></div>' +
          '<div class="frc-signal note">This checker needs enough text for its patterns to mean anything. Very short reviews are also a weak signal on their own: real experiences usually include at least one specific.</div></div>';
        return;
      }
