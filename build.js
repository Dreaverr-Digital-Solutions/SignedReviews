...
// calls identify()). Session recording is ON with maskAllInputs (form fields —
// passwords/emails/payment — are never captured); first-party cookie on the
// registered domain enables the landing→app funnel stitch across subdomains.
// Recording also requires "Record user sessions" to be enabled in the PostHog
// project settings — the client flag alone does not start replay.
...
const POSTHOG_SNIPPET = POSTHOG_KEY
  ...
  <script>
    ...
    posthog.init('${POSTHOG_KEY}',{api_host:'${POSTHOG_HOST}',person_profiles:'identified_only',session_recording:{maskAllInputs:true},respect_dnt:true});
  </script>`
  : '';
